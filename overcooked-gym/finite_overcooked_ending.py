...
class Overcooked(Env):
    ...
    def __init__(self, layout="small", id="Overcooked-v2", max_timesteps=float("inf"), rewards=(0, 0, 0, 1),
                 ending="time"):
        ...
        if ending not in ["time", "onion", "soup_ready", "pickup", "deliver"]:
            raise ValueError("Ending must be one of the following strings: 'time', 'onion', 'pickup' or 'deliver'.")
        self.ending = ending
    ...
    def transition(self, state, joint_action):
        ...
        def interact(object_row, object_column, hand, next_pan, next_balconies, reward):
            ...
            return next_hand, next_pan, next_balconies, reward, done
        ...
        def step_agent(row, column, heading, hand, action, action_meaning, teammate_row, teammate_column, next_pan, next_balconies, reward):
            ...
            if moving and heading != action:
                ...
                next_hand, next_pan, next_balconies, reward, done = interact(object_row, object_column, hand, next_pan, next_balconies, reward)

            return next_row, next_column, next_heading, next_hand, next_pan, next_balconies, reward, done

        next_pan, next_balconies = pan, balcony_contents
        reward = self.move_reward

        next_a0_row, next_a0_column, next_a0_heading, next_a0_hand, next_pan, next_balconies, reward0, done0 = step_agent(a0_row, a0_column, a0_heading, a0_hand, a0, a0_meaning, a1_row, a1_column, next_pan, next_balconies, reward)
        next_a1_row, next_a1_column, next_a1_heading, next_a1_hand, next_pan, next_balconies, reward1, done1 = step_agent(a1_row, a1_column, a1_heading, a1_hand, a1, a1_meaning, next_a0_row, next_a0_column, next_pan, next_balconies, reward)

        terminal = (self._timestep >= self.max_timesteps or done0 or done1)

        cooking = pan == THREE_ONIONS
        if cooking:
            next_pan = SOUP_READY
            if self.ending == "soup_ready":
                terminal = True
